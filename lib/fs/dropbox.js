...
ReadStreamWrapper.prototype.destroy = function (callback) {
  this.stream.destroy(function (error) {
    if (callback) callback(error ? new util.CustomError("UnknownError") : null)
  })
}
...
WriteStream.prototype.write = function (data, callback) {
  ...
  if (callback) util.async(callback)(null, data.length)
...
WriteStream.prototype.destroy = function (callback) {
  if (callback) util.async(callback)(null)
...
DropboxFileSystem.prototype.download = function (link, destination, callback) {
  var self = this
