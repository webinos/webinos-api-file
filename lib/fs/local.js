...
LocalFileSystem.httpHandler = function (request, response) {
  var ref = url.parse(request.url, true).query["ref"]
  if (links[ref]) {
    send(request, links[ref]).pipe(response)
  } else {
...
LocalFileSystem.prototype.createFile = function (path, exclusive, callback) {
  ...
  fs.stat(self.realize(path), function (error1, stats) {
    switch (error1 ? error1.code : null) { // (error1 && error1.code) || "EEXIST"
      case null: // "EEXIST"
        if (exclusive) {
...
LocalFileSystem.prototype.createFileLink = function (path, callback) {
  ...
  var link = url.format(
    { protocol : "http"
    , hostname : LocalFileSystem.hostname
    , port : LocalFileSystem.port
    , pathname : "/module/webinos-api-file/link"
    , query : { "ref" : ref }
    })
  util.async(callback)(null, link)
}

// TODO Inherit new readable stream.
...
ReadStreamWrapper.prototype.destroy = function (callback) {
  this.stream.destroy(function (error) {
    if (callback) callback(error ? map(error) : null)
  })
}
...
// TODO Inherit new writable stream.
...
LocalFileSystem.prototype.download = function (link, destination, callback) {
  var self = this
  self.createWriteStream(destination, null, function (error, stream) {
