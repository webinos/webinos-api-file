...
// TODO A move of a file on top of an existing file must attempt to delete and replace that file.
// TODO A move of a directory on top of an existing empty directory must attempt to delete and replace that directory.
...
// TODO A copy of a file on top of an existing file must attempt to delete and replace that file.
// TODO A copy of a directory on top of an existing empty directory must attempt to delete and replace that directory.
...
// webinos <3 webkit
function verifyDestinationPathForDownload(parent, name) {
  return parent.isDirectory
}

// webinos <3 webkit
function getDestinationPathForDownload(parent, name) {
  return vpath.join(parent.fullPath, name)
}

VirtualFileSystem.prototype.download = function (link, parent, name, callback) {
  if (!verifyDestinationPathForDownload(parent, name)) {
    return callback(new util.CustomError("InvalidModificationError"))
  }

  var destinationPath = getDestinationPathForDownload(parent, name)
  this.fs.download(link, destinationPath, function (error) {
    if (error) return callback(error)
    callback(null, new FileEntry(parent.filesystem, destinationPath))
  })
}
